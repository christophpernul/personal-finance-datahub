...
if __name__ == "__main__":
    ...
    filepath_toshl_cashflow = (
        DATAHUB_TRANSFORM_FILEPATH / DATAHUB_CONFIG_CASHFLOW_META["filePath"]
    )
    stage = "a_00"
    filepath = filepath_toshl_cashflow / (
        f"{stage}_" + DATAHUB_CONFIG_CASHFLOW_META["transformFileName"]
    )
    a_00_cashflow = update_toshl_cashflow(
        datahub_source_root_path=DATAHUB_SOURCE_FILEPATH,
        datahub_cashflow_config=DATAHUB_CONFIG_CASHFLOW_META,
    )
    save_data(
        data=a_00_cashflow.reset_index(),
        filepath=filepath,
    )
    ...
    save_data(
        data=a_11_incomes.reset_index(),
        filepath=filepath_incomes,
    )
    save_data(
        data=a_10_expenses.reset_index(),
    ...
    save_data(
        data=a_20_expenses.reset_index(),
        filepath=filepath,
    )
